...
const HERO_INTRO_HANDOFF_PROGRESS = 0.35;
const HERO_INTRO_HANDOFF_DELAY_FALLBACK = 1.2;
...
function withMotionDistance(props: { x: number; y: number }, distance: number | undefined) {
  if (distance === undefined) return props;
  if (props.x !== 0) return { x: Math.sign(props.x) * distance, y: 0 };
  if (props.y !== 0) return { x: 0, y: Math.sign(props.y) * distance };
  return props;
}
...
function isRenderedElement(el: Element) {
  const rect = el.getBoundingClientRect();
  const style = window.getComputedStyle(el);
  return rect.width > 0 && rect.height > 0 && style.display !== "none";
}

function getHeroIntroHandoffDelay() {
  const introEls = Array.from(document.querySelectorAll<HTMLElement>("[data-hero-intro]"));
  const renderedIntroEls = introEls.filter(isRenderedElement);
  const candidates = renderedIntroEls.length ? renderedIntroEls : introEls;

  const handoffDelay = candidates.reduce((longest, el) => {
    const delay = parseNumber(el.dataset.kvDelay, 0);
    const duration = parseNumber(el.dataset.kvDuration, 0.9);
    return Math.max(longest, delay + duration * HERO_INTRO_HANDOFF_PROGRESS);
  }, 0);

  return handoffDelay || HERO_INTRO_HANDOFF_DELAY_FALLBACK;
}
...
export function initScrollMotion() {
  ...
  if (!document.querySelector("[data-kv-from]")) {
    ...
    const gatedRevealCalls: gsap.core.Tween[] = [];
    ...
    const ctx = gsap.context(() => {
      gsap.utils.toArray<HTMLElement>("[data-kv-from]").forEach((el) => {
        const dir = withMotionDistance(directionProps(el.dataset.kvFrom, KV_DIRECTIONS, "up"), parseOptionalNumber(el.dataset.kvDistance));
        const delay = parseNumber(el.dataset.kvDelay, 0);
        const duration = parseNumber(el.dataset.kvDuration, 0.9);
      ...
      const heroIntroHandoffDelay = getHeroIntroHandoffDelay();
      ...
      gsap.utils.toArray<HTMLElement>("[data-reveal-stagger]").forEach((container) => {
        ...
        const duration = parseNumber(container.dataset.revealDuration, 0.64);
        const waitForHeroIntro = container.dataset.revealAfter === "hero-intro";

        initialized.push(container);
        container.setAttribute("data-reveal-init", "");

        const playAfterHeroIntro = (tl: gsap.core.Timeline) => {
          const remaining = Math.max(0, heroIntroHandoffDelay - (performance.now() - motionStartedAt) / 1000);
          if (remaining === 0) {
            tl.play();
            return;
          }

          const delayedCall = gsap.delayedCall(remaining, () => {
            tl.play();
          });
          gatedRevealCalls.push(delayedCall);
        };
        ...
        const dir = withMotionDistance(directionProps(variant, REVEAL_DIRECTIONS, "up"), parseOptionalNumber(container.dataset.revealDistance));
        ...
        if (waitForHeroIntro) {
          ScrollTrigger.create({
            trigger: container,
            start,
            onEnter: () => playAfterHeroIntro(tl),
            onEnterBack: () => playAfterHeroIntro(tl),
            onLeaveBack: () => {
              if (useReverse) {
                tl.reverse();
              }
      ...
      if (header && headerLogo) {
        ...
        if (window.scrollY < 24) {
          gsap.fromTo(
            headerLogo,
            { autoAlpha: 0, x: -getHeroLogoDistance(), y: 0 },
            {
              autoAlpha: 1,
              x: 0,
              y: 0,
          ...
          gsap.set(headerLogo, { autoAlpha: 1, x: 0, y: 0, force3D: true });
        }
    ...
    const cleanup = () => {
      ...
      gatedRevealCalls.forEach((call) => call.kill());
